...
let todaysDateEl = document.querySelector("#todays-date");
...
function getWeather(e) {
  e.preventDefault();
  let city = cityInputEl.value.toUpperCase().trim();
  if (!city) {
    alert("Please enter a City");
  } else {
    getCityWeather(city);
    getFiveDay(city);
    // move city data to front of cityHistoryEl array
...
function printWeather(weather) {
  ...
  citySearchInputEl.setAttribute(
    "href",
  ...
  citySearchInputEl.textContent = weather.name + ", " + weather.sys.country;
  // add/remove date el
  todaysDateEl.textContent = "";
  let currentDate = document.createElement("span");
  currentDate.textContent =
    "   -   " + moment(weather.dt.value).format("dddd MMM D");
  todaysDateEl.appendChild(currentDate);
  ...
  // append icon
  todaysDateEl.appendChild(wIcon);
  console.log(weather);
  // create el for temp data
  let tempEl = document.createElement("span");
  tempEl.textContent = "Temperature: " + Math.round(weather.main.temp) + "°F";
...
function printForecast(weather) {
  ...
  // loop to print weather for next 5 days
  for (let i = 5; i < forecast.length; i = i + 8) {
    let dailyForecast = forecast[i];
    let forecastEl = document.createElement("div");
    forecastEl.classList = "card bg-info text-light m-2 p-4";
    //create date el
